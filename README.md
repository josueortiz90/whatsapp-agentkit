...
- 🏢 **Multi-tenant con feature flag (opcional)** — Una sola instancia puede atender VARIOS negocios. Activado con `MULTI_TENANT=true`: webhooks ruteados por slug (`/webhook/<slug>`), dashboards aislados por tenant (`/dashboard/t/<slug>/`), admin UI en `/admin` para crear/editar tenants. Cada tenant tiene sus propios tokens (Whapi + Anthropic), sus propios prompts/knowledge en `tenants/<slug>/`, sus propias credenciales de dashboard. Aislamiento de datos por `tenant_id` en la BD. Si `MULTI_TENANT=false`, el agente sigue igual que en single-tenant.
